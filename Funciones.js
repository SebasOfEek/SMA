    ...
    if ( form.elements[i].type == 'select-one' ||
	  ...
	  if ( !sw && form.elements[i].type == 'radio' )
	  {
		    ...
		    if ( !sw )
		    {
		 	     ...
		 	     showLabelError('tdError','Por favor complete informacion.'+ '\n' + 'Ingrese ' + form.elements[i].title);
				 try
				 {
				    //form.elements[i].focus();
				 }
...
{ 
  ...
  {   
      ...
      {                                   
          ...
          {              
              show_message('Por favor complete informacion.'+ '\n' + 'Ingrese ' + inputs[i].title);
              sw = false;
              break;
          }
      }    
...
function makeBarInformation(wth,ptitle){
  ...
  if (!ptitle) title = '&nbsp;'; else title = ptitle;
...
function makeTitleBarII(wh,title){
  ...
  try{
    ...
    str += '    <td width="30" height="23" background="'+Rutav+'/vista/img/barlineder.gif">&nbsp;</td>';*/
  	...
  	str += '    <td width="30" height="23" background="'+Rutav+'/vista/img/titlebar.png">&nbsp;</td>';
    str += '  </tr>';
	str += '</table>';	
    document.write(str);
  }catch(theException){
...
function drawCommandBottonII(arrCmd,caption){
  ...
  try{
    ...
    if (arrCmd.length > 0){
      if ( caption )w = 100 * arrCmd.length; else	w = 35 * arrCmd.length;
      ...
      for (i = 0; i < arrCmd.length; i++){
	    ...
	    if ( caption ) strCaption = '&nbsp;' + arrCmd[i][0];
...
function clearFields(args,op){
  ...
  if ( op ){
    if (form.state) form.state.value = 'New';
    form.submit();
	return;
  }
  
  if ( form.varAccess ) varAccess = form.varAccess.value;
  if ( varAccess != '' ){
    if ( form.btnSearch ){
	  if(varAccess.charAt(0) == '1') form.btnSearch.disabled = 0; else form.btnSearch.disabled = 1;
	}

    if ( form.btnSave ){
  	  if(varAccess.charAt(1) == '1') form.btnSave.disabled = 0; else form.btnSave.disabled = 1;
      if(varAccess.charAt(2) == '1') 
	    if (form.btnSearch) form.btnSearch.disabled = 0;
	}

    if ( form.btnDelete ) form.btnDelete.disabled = 1;
  }
  ...
  for (i = 0; i < form.elements.length; i++){
    if ( form.elements[i].type == 'text'     ||
	  ...
	  if (!sw) form.elements[i].value = ''; 
	}
  }// end for
  if (form.state) form.state.value = 'Insert';
}
<!-- ***************************************************************************** -->
      ...
      if (format){
        if (format == 'Date'){ // Formato de fecha
          if (valor.length >= 8){
            ...
            if(cYear.length == 2){
              if(parseInt(cYear.substring(0,1)) >= 6) cYear = '19' + cYear; else cYear = '20' + cYear;	   
  ...
  for(i = 0; i < element.length; i++){
    if (element[i].type == 'checkbox'){
      if (name){
	    ...
	    if (sw != -1) element[i].checked = sw; else element[i].checked = !element[i].checked;
      }
	}
  }	
}
...
function show_message(msg,callback) {    
	...
	*/
function show_message_custom(type, msg,callback) {    		
		//type : 0,Informaci�n #1, Exito #2, Error#3, Confirmaci�n
		var width_ = msg.length * 9;		
		if(width_ > 600) width_ = 600;	
		...
		$("#dialog-message").dialog("destroy");				   	    		
			...
			$("#dialog-message").dialog({
				modal	   : true,
				dialogClass: 'dialog-message-custom',
				resizable  : false,			
				buttons    : {
					Ok: function() {
						$(this).dialog('close');
		...
		if(type){
			...
			$('.dialog-message-custom .ui-widget-header').css('background-image','url("")');
			switch(type){
				case '1':
					$("#dialog-message").attr("title", "EXITO!");
					$(".dialog-message-custom .ui-widget-header").css("background-color", "green");
				break;
				case '2':
					$("#dialog-message").dialog("option", 'title', "ERROR !");
					$(".dialog-message-custom .ui-widget-header").css("background-color", "red");
				break;
			}	
		}			
	}
...
{   
    ...
    if( height_ == '100%' ) height_ = window.top.get('display').offsetHeight - 40;

    if(!width_ ) width_  = 'auto';
    if(!height_) height_ = 'auto';
...
{   
    ...
    if( height_ == '100%' ) height_ = window.top.get('display').offsetHeight - 25;
    if( width_  == '100%' ) width_  = window.top.get('display').offsetWidth - 12;
    

    
    if(!position_) position_ = 'top';
...
function conf_panel_old(width_,height_,id,position_,noBoton)
{   
    if(!id) id = 'dialog';
...
{   
    ...
    ({
		...
		yearRange       : 'c-25:c+25',
		changeYear      : true,
		onSelect        : function() { eval(funcion); } 
	});	
...
{ 
  $("input:button,input:submit,button", ".demo").button();
}  
/**/
...
function validRequiredAllFields(form)
{
  ...
  if( !exito )
  {
	 strValid = "Por favor debe completar la siguiente informacion:"+strValid;
	 if( countElements > 0 )
	 {
   ...
   {    	     
		html += '<tr>'       +
				   ...
				   '<td align="right">'    +
				    ...
				    '<input name="arransQAC' + nroqst[i] + '" id="arransQAC' + vecscv[i] + '" type="hidden" value="">'
				   '</td>'   +
				 '</tr>' ;	  			
	}
	html += '</table>';
...
function getTimeServer(fchcls,maxmin,nameForm,url){
  ...
  if (dateServer != ''){
    ...
    if (nHor < 10) nHor = '0' + nHor;
    if (nMin < 10) nMin = '0' + nMin;
    if (nSeg < 10) nSeg = '0' + nSeg;

    if (nHorCmtr < 10) nHorCmtr = '0' + nHorCmtr;
    if (nMinCmtr < 10) nMinCmtr = '0' + nMinCmtr;
    if (nSegCmtr < 10) nSegCmtr = '0' + nSegCmtr;
    ...
    if ((fchcls)&&(nameForm)&&(url))
      setTimeout("getTimeServer('"+fchcls+"',"+maxmin+",'"+nameForm+"','"+url+"')",99999999);
	else
      setTimeout('getTimeServer()',99999999);
  }
}
...
{   
	...
	if(!img || $.trim(img)==''){
	 ...
	 $('#' + id).datepicker({
				...
				numberOfMonths  : months,
				changeYear      : true,
				onSelect        : function() { eval(funcionClick); },
				onClose			: function() { eval(funcionCallback); }				
	 }
	 );
	 return;
...
function show_message(msg,callback,tipo) {    
	...
	nuevaVentana('dialog-message','Informaci�n','','');
	document.getElementById('dialog-message').innerHTML = msg;
	...
	$("#dialog-message").dialog("option", "close",
			function(event, ui) 
		       {
	                 if(callback)
				   eval(callback);
	...
	$("#dialog-message").find('button[title="close"]').remove();
	
	var span=$(".ui-dialog-titlebar-close");
	...
	if(!tipo)
		 $("#dialog-message").parents(".ui-dialog:first").find(".ui-dialog-titlebar").addClass("ui-state-highlight");
	else
	if(tipo.toUpperCase()=='ERROR')
		$("#dialog-message").parents(".ui-dialog:first").find(".ui-dialog-titlebar").addClass("ui-state-error").removeClass("ui-state-highlight");
		else
		$("#dialog-message").parents(".ui-dialog:first").find(".ui-dialog-titlebar").addClass("ui-state-highlight").removeClass("ui-state-error");
...
function conf_dialog(id,width_,height_,position_){	
    ...
    if( height_ == '100%' || $.trim(height_) =='') height_ = document.body.clientHeight - 25;
    if( width_  == '100%' || $.trim(width_) =='') width_  = document.body.clientWidth-12;
      
    if(!position_ || $.trim(position_) =='') position_ = 'top';
...
function showWindowSearch(pKey,fieldReturn,searchFlag,filter,callBackSearch){
	...
	//filter: fields to conform the statement where  001|OOO5 -> FALSE ,  &nrofkp&|&nroklt& -> TRUE $('#'+ARR[i]).val()
	  ...
	  try{
		    nuevaVentana('windowsSearch','VENTANA DE BUSQUEDAS','auto','auto');
		  ...
		  // TODO: handle exception
		      show_message("<center class=\"caption tablas \" style=\height: 100%;font-size: medium;\" >" +"¡No existe el marco de busqueda, Consulte a su proovedor!"+"</center>",'','ERROR');
		    return;    
	       }
    ...
    /** FIN FUNCIONES  PARA SELEC DEL NUEVO CONFIG GRID **/
     ...
     function putTotal()
     {
  	  ...
  	  {
  	  	   var filas = get("GridTable").rows;
  		   var arrDbt = getByName("dbtmvx");
  		   var arrCrd = getByName("crdmvx");   
  		   for( var i = 0; i < filas.length; i++ )
  		   {
  		   ...
  		   //Total
  		   var fila = get("GridTable").insertRow(-1);
  		   ...
  		   fila = get("GridTable").insertRow(-1);
	...
	function conf_tab(id){
    	...
    	function validarFieldByForm(divForm,selector){
   		 var flag = true;
   		 
   		$('#'+divForm).find(selector).each
   		 ( function()
   	           {
   	                 if( $(this).val()  == '' ||  $(this).val()  == '-9' ||  $(this).val()  == '000'  ||  $(this).val()  == 'Seleccione..'   ||  $(this).val()  == 'Seleccione'  )
   		         			{	
...
function showWindowWaitTwo(w,h,r,texto,url,prm,s,name,full){
	  var output = '', pscroll = 'no';
	  var x = (screen.Width - w) / 2;
	  var y = (screen.Height - h) / 2;
	...
	/*  
	  ...
	  */
	  ...
	  newWindowReport.document.open();
	  newWindowReport.document.write(output);
	  var doc_new = newWindowReport.document;
	  var cont = 0;
	  $(doc_new).ready(function(){
		  if(cont == 0){
			  $.ajax({
				  type:'post',
				  url:url,
			  success:function(response,txt)
			  		{
				  		var content = $('<iframe src="'+url+'" name="SubHtml" width="100%" height="100%" scrolling="auto" frameborder="1">'+
				  							'<table width="100%" height="100%" align="center" cellpadding="0" cellspacing="0" class="tablas">'+
				  								'<tr valign="middle">'+
				  									'<td class="caption" align="center">'+texto+'</td>'+
				  								'</tr>'+
			  								'</table>'+
			  								'</iframe>');
				  		$(doc_new).find('body').append(content);
			  		},
			  	error:function(response,tst)
			  		{
			  			var contError = $('<table width="100%" height="100%" align="center" cellpadding="0" cellspacing="0">'+
		  								  	'<tr>'+
		  								  		'<td align="center" valign="middle">'+
		  								  			'<table width="400" border="0" cellspacing="1" cellpadding="1" class="tablas">'+
		  								  				'<tr>'+
		  								  					'<td width = "400" colspan = "2" class = "titlebar_">Error</td>'+
		  								  				'</tr>'+
		  								  				'<tr>'+
		  								  					'<td width="400" colspan="2">&nbsp;</td>'+
		  								  				'</tr>'+
		  								  				'<tr>'+
		  								  					'<td width="350" valign="top"><div align="center" class="caption">Documento no Encontrado</div></td>'+
		  								  				'</tr>'+
			  								  			'<tr>'+
										  					'<td width="400" colspan="2">&nbsp;</td>'+
										  				'</tr>'+
										  				'<tr>'+
										  					'<td width = "400" colspan = "2" class = "titlebar_"></td>'+
										  				'</tr>'+
										  			'</table>'+
		  								  		'</td>'+
		  								  	'</tr>'+
		  								  '</table>');	  			  
			  			$(doc_new).find('body').append(contError);
			  		}
			  });  
			cont++;  
		  }
...
function validarAllform(id, notfld) {
    ...
    if (!notfld) {
        $('#' + id + ' input[type="text"],#' + id + ' input[type="checkbox"],#' + id + ' input[type="radio"],#' + id + ' select, #' + id + ' textarea').each(function() {
            if ($(this).val() === "") {
                flag = false;
        ...
        $('#' + id + ' input[type="text"],#' + id + ' input[type="checkbox"],#' + id + ' input[type="radio"],#' + id + ' select, #' + id + ' textarea').not(notfld).each(function() {
            if ($(this).val() === "") {
                flag = false;
                mensaje += "," + $(this).attr('title');
                $(this).css('border', 'solid 2px red');
...
function show_confirmation(msg, callbackA, callbackC)
{
    ...
    var cerrarC = true;
    $("#dialog-confirm").remove();
    nuevaVentana('dialog-confirm', 'Confirmaci?n', '', '');
 ...
 $("#dialog-confirm").dialog({
    ...
    close: function(event, ui) 
            {
     if (callbackA && !cerrarA) {
         if ($.type(callbackA) === 'function')
      callbackA();
           else
                          eval(callbackA);
    } else if (callbackC && !cerrarC) {
        if ($.type(callbackC) === 'function')
     callbackC();
        else
     eval(callbackC);
    }
...
function showLoader(mensaje, image) {
    ...
    $("#jquery-loader").css('background-color', 'white');
